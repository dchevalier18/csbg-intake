...
export async function addUser(name: string, username: string, role: string): Promise<ActionResult> {
  ...
  const taken = (await db.select().from(t.users).where(eq(t.users.username, uname)))[0];
  ...
  await audit(admin.id, "user.add", "user", id, `${cleanName} (${role}) · @${uname}`);
...
export async function updateUserRole(id: string, role: string): Promise<ActionResult> {
  const admin = await requireAdmin();
  if (!(ROLES as readonly string[]).includes(role)) return { ok: false, message: "Pick a valid role." };
  const u = (await db.select().from(t.users).where(eq(t.users.id, id)))[0];
  if (!u || !u.active) return { ok: false, message: "User not found." };
  await db.update(t.users).set({ role }).where(eq(t.users.id, id));
  await audit(admin.id, "user.role", "user", id, `${u.name}: ${u.role} → ${role}`);
  revalidatePath("/", "layout");
  return { ok: true };
}
...
export async function setUserAccess(id: string, all: boolean): Promise<ActionResult> {
  ...
  const u = (await db.select().from(t.users).where(eq(t.users.id, id)))[0];
  if (!u || !u.active) return { ok: false, message: "User not found." };
  await db.update(t.users).set({ access: all ? "all" : "assigned" }).where(eq(t.users.id, id));
  await audit(admin.id, "user.access", "user", id, `${u.name}: ${all ? "all programs" : "assigned programs only"}`);
  revalidatePath("/", "layout");
  return { ok: true };
}
...
export async function toggleUserProgram(userId: string, programId: string): Promise<ActionResult> {
  const admin = await requireAdmin();
  const u = (await db.select().from(t.users).where(eq(t.users.id, userId)))[0];
  if (!u || !u.active) return { ok: false, message: "User not found." };
  const p = (await db.select().from(t.programs).where(eq(t.programs.id, programId)))[0];
  if (!p) return { ok: false, message: "Program not found." };
  const existing = (await db.select().from(t.userPrograms)
    .where(and(eq(t.userPrograms.userId, userId), eq(t.userPrograms.programId, programId))))[0];
  if (existing) {
    await db.delete(t.userPrograms)
      .where(and(eq(t.userPrograms.userId, userId), eq(t.userPrograms.programId, programId)));
    await audit(admin.id, "user.program.unassign", "user", userId, `${u.name} − ${p.short}`);
  } else {
    await db.insert(t.userPrograms).values({ userId, programId });
    await audit(admin.id, "user.program.assign", "user", userId, `${u.name} + ${p.short}`);
  }
  revalidatePath("/", "layout");
  return { ok: true };
}
...
export async function removeUser(id: string): Promise<ActionResult> {
  ...
  const u = (await db.select().from(t.users).where(eq(t.users.id, id)))[0];
  if (!u || !u.active) return { ok: false, message: "User not found." };
  await db.update(t.users).set({ active: 0 }).where(eq(t.users.id, id));
  await db.delete(t.sessions).where(eq(t.sessions.userId, id));
  await audit(admin.id, "user.remove", "user", id, u.name);
  revalidatePath("/", "layout");
