...
export async function setCsbgCeiling(ceiling: number): Promise<ActionResult> {
  ...
  db.update(t.organization).set({ csbgCeiling: c }).where(eq(t.organization.id, 1)).run();
  audit(admin.id, "org.ceiling", "organization", "1", `CSBG income ceiling → ${c}% FPL`);
  revalidatePath("/", "layout");
  return { ok: true };
}
...
export async function publishFpl(year: number, base: number, perAdditional: number): Promise<ActionResult> {
  ...
  const existing = db.select().from(t.fplSchedules).where(eq(t.fplSchedules.year, y)).get();
  if (existing) return { ok: false, message: `FPL ${y} already exists in the guideline history.` };
  db.update(t.fplSchedules).set({ status: "archived" }).run();
  db.insert(t.fplSchedules).values({ year: y, base: b, perAdditional: p, effective: todayIso(), status: "active" }).run();
  const clientCount = db.select({ id: t.clients.id }).from(t.clients).all().length;
  const openApps = db.select({ stage: t.applications.stage }).from(t.applications).all()
    .filter((a) => (OPEN_STAGES as readonly string[]).includes(a.stage)).length;
  const n = clientCount + openApps;
  audit(admin.id, "fpl.publish", "fpl", String(y), `Published FPL ${y} — household of 1 $${b}, each additional $${p}; ${n} cases stay pinned`);
  revalidatePath("/", "layout");
...
export async function makeFplActive(year: number): Promise<ActionResult> {
  const admin = await requireAdmin();
  const y = Math.round(Number(year));
  const target = db.select().from(t.fplSchedules).where(eq(t.fplSchedules.year, y)).get();
  if (!target) return { ok: false, message: "That guideline year doesn't exist." };
  db.update(t.fplSchedules).set({ status: "archived" }).run();
  db.update(t.fplSchedules).set({ status: "active" }).where(eq(t.fplSchedules.year, y)).run();
  audit(admin.id, "fpl.activate", "fpl", String(y), `FPL ${y} re-activated for new assessments`);
  revalidatePath("/", "layout");
